...
C.append(md("""**Interpretation.** Purchase amounts are spread almost uniformly between ~\\$80 and \\$500 with no typical spend level, and the per-category boxplots show similar medians and ranges with no extreme outliers. The correlation heatmap (IDs excluded; engineered features included) confirms this flatness: apart from mechanical relationships among the engineered aggregates (`total_spend` ~ `avg_spend` x `n_transactions`), every off-diagonal correlation is near zero. In particular the social-side predictors (`engagement_score`, `purchase_interest_score`) are uncorrelated with purchase behaviour, an early warning, consistent with the baseline check in Task 4, that this dataset carries little signal for product prediction."""))

C.append(md("""## Task 2: Image Collection, Augmentation & Features
3 expressions per identity (neutral / smiling / surprised); 3 augmentations per image (rotation+brightness, horizontal flip, grayscale); features = 32-bin grayscale histogram + 16x16 pixel embedding -> `image_features.csv`."""))
...
Two phrases per identity ("Yes, approve", "Confirm transaction"); waveform + spectrogram per raw sample; 3 augmentations (pitch shift +2 st, time stretch 0.85x, additive noise); features = 13 MFCC means + 13 MFCC stds + spectral roll-off + spectral centroid + RMS energy + zero-crossing rate -> `audio_features.csv`."""))
